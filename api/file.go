...
func renameFile(user model.User, id string, filename string) {
	...
	CallAPIMethod("PATCH", model.FileEndPointRoot, endpoint, user.AccessToken, "", content, "application/json")
...
func UpdateFile(user model.User, id string, fileName string, fileContent []byte, fileContentType string) (string, int) {
	...
	// Upload the file
	rsp, code := CallAPIMethod("PUT",  model.FileEndPointRoot,endpoint, user.AccessToken, "", fileContent, fileContentType)
...
func GetKeywordFiles(user model.User, searchKeyword string, query string) model.Files {
	...
	filesResponse, _ := CallAPIMethod("GET", model.FileEndPointRoot, endpoint, user.AccessToken, query, nil, "")
...
func LiveDownloadFile(user model.User, fileID string) {
	...
	driveItemResponse, _ := CallAPIMethod("GET",  model.FileEndPointRoot,endpoint, user.AccessToken, "", nil, "")
